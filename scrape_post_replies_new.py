...
OUTPUT_EXCEL = "output/uco_complete_data_12_05_2026_to_10_06_2026.xlsx"

OUTPUT_CSV = "output/uco_complete_data_12_05_2026_to_10_06_2026.csv"

os.makedirs("output", exist_ok=True)

all_data = []

# Tracker for batch saving replies efficiently
reply_records_since_save = 0
...
def save_incremental():
    ...
    try:
        ...
        print(
            f"\n[Auto Saved]: {len(df)} records safely synced to files."
        )
...
with sync_playwright() as p:
    ...
    for idx, post_url in enumerate(post_urls):
        ...
        try:
            ...
            try:
                ...
                analytics_locator = page.locator(
                    "a[href*='/analytics']"
                )

                if analytics_locator.count() > 0:

                    analytics_text = analytics_locator.first.inner_text().strip()

                    if analytics_text:

                        views = analytics_text.split()[0]

                if not views:

                    all_spans = page.locator(
                        "span"
                    )
            ...
            # Save immediately following each newly documented primary POST
            save_incremental()
            ...
            for j in range(1, reply_count):
                ...
                try:
                    ...
                    # Batch performance logic: Incremental auto-save after every 10 replies
                    reply_records_since_save += 1
                    if reply_records_since_save >= 10:
                        save_incremental()
                        reply_records_since_save = 0
    ...
    # =====================================
    # SAVE OUTPUT (Final Master Clean)
    # =====================================

    # Execute one definitive save pass to sweep up any remaining buffered items
    save_incremental()
    ...
    print(
        f"\nTotal Dataset Records: {len(all_data)}"
    )

    print(
        f"\nExcel File Output Path: {OUTPUT_EXCEL}"
    )

    print(
        f"CSV File Output Path:   {OUTPUT_CSV}"
    )
